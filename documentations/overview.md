...
- Giữa: CanvasBar — search · Ask Dolphin · VI|EN · user **Hà** (`/avatars/ha.webp`). Canvas + chat + seed **VI**.
...
| Nav | Canvas |
| --- | --- |
| Dashboard | KPI demo · chart doanh thu · tasks · 5 khối list · timeline (avatar staff) · gợi ý AI (`lib/dashboard-demo.ts`) |
| Khóa học | KPI demo (`courses-demo.ts`) · bảng filter + mã demo · panel tab Tổng quan / Lớp / Lịch sử / Tài liệu · roster % điểm danh + TT · avatar GV |
| Lớp học | KPI demo (`classes-demo.ts`) · timeline hôm nay · bảng + % điểm danh · panel tab Tổng quan / Lịch / Điểm danh / Lịch sử · roster TT · avatar GV |
| Học viên | KPI demo (`students-demo.ts`) · phân khúc + chart · bảng chuyên cần/TT · panel profile + tab · auto chọn HV đầu · avatar GV phụ trách |
| Giáo viên | KPI demo (`teachers-demo.ts`) · timeline hôm nay · bảng + rating/ca rảnh · panel tab Tổng quan / Lịch dạy / Lịch sử / Hiệu suất · khóa gán + CTA · avatar (ảnh 5 người / chữ cái GV khác) |
...
| Tác vụ | KPI từ seed (`tasks-demo.ts`) · thanh lọc kiểu Jira · avatar staff · panel tab **Tổng quan** (mô tả, thông tin, checklist, cập nhật, 1 comment mới nhất + Xem thêm) / **Comment** (thread gọn + composer 1 khung) |
